...
| スキル | 種別 | 役割 | 生成する？ |
|--------|------|------|-----------|
| `ordito-update-block` | 書き込み | IR の1ブロックを差分更新し `updated_at` を進める | **しない** |
...
「書き込みは生成を引き起こさない」の実装。各スキルの発火条件と I/O は
...
**安定版 — 仕様 v1.0。** 二段階で構築・検証（単一ページ → 複数ページ＋コレクション＋混在生成）し、差分更新・二段確認を
...
生成に使っている。**未カバー:** 既存ドキュメントからの実データ移行（雑多な入力では未検証）、マルチライタ同時更新、

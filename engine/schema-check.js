...
//   schemas/ordito-v0.3.schema.json の $defs（document / collection / block …）を
...
const SCHEMA_FILE = path.join(__dirname, "..", "schemas", "ordito-v0.3.schema.json");
...
// ---- CLI: node engine/schema-check.js <file.json> <document|collection|block> ----
if (require.main === module) {
  const [file, defName = "document"] = process.argv.slice(2);
  if (!file) { console.error("usage: node engine/schema-check.js <file.json> [document|collection|block]"); process.exit(1); }

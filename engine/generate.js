...
//     node engine/generate.js                              # mode=deterministic
// =========================================================================
...
function parseArgs(argv) {
  const opts = { mode: "deterministic", out: path.join(ROOT, "dist"), strategy: "schema",
    collection: null, irDir: null, aiCache: null, model: null, docs: [] };
...
async function runCollection(opts, contract, frame, styles) {
  ...
  console.log(`Ordito engine — collection=${collection.collection_id} mode=${opts.mode} 出力=${path.relative(ROOT, opts.out)}/`);
  const report = [];
